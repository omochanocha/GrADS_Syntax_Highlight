...
本拡張機能は`.gs`ファイルに対してSyntax Highlightを提供するものである。ただしあくまでSyntax Highlightのみであるので、**そのコードの正しさは保証できない**(つまり、例えばある行がコメントアウトされていてもその行が実行されてしまう可能性もある)のでGrADSで実行する前に必ず各自で確認していただきたい。
